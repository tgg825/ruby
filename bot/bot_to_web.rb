...
class BotServlet < WEBrick::HTTPServlet::AbstractServlet
    ...
    # and a text entry box for the user to converse with the bot.  It
    # uses some placeholder text (%RESPONSE%) so the bot's responses
    # can be substitued easily later on.  
    @@html = %q{
        <html><body>
        <form method="get">
        <h1>Talk To A Bot</h1>
    ...
    def do_GET(request, response)
        #Mark the request as sucessful and set MIME type to support HTML
        response.status = 200
        ...
        # Format the text and substitue it into the HTML template
